from math import exp

h   = 6.6260755e-27    # planck's constant (ergs-s)
c   = 2.99792458e10    # speed of light (cm/s)
k   = 1.380658e-16     # boltzmann constant (ergs/K)
k_ev = 8.6173324e-5    # boltzmann constant (ev/K)
pi  = 3.14159          # just pi
sb  = 5.6704e-5        # stefan boltzman constant (ergs cm^-2 s^-1 K^-4) 
m_e = 9.10938188e-28   # mass of electron (g)
m_p = 1.67262158e-24   # mass of proton (g)
a   = 7.5657e-15       # radiation constant
e   = 4.8032e-10       # electron charge in cgs
a0  = 0.53e-8          # bohr radius (cm)
rydberg = 2.1798741e-11 # rydberg constant in ergs
G   = 6.674e-8


sigma_t   = 6.6523e-25       # thomson cross-section in cm^2
sigma_tot = 0.0265400193567  # integrated line coefficent (cm^2 Hz)

# unit conversions
ev_to_ergs  = 1.60217646e-12
ergs_to_ev  = 6.24150974e11
cm_to_angs  = 1.0e8
angs_to_cm  = 1.0e-8
day_to_sec  = 86400.0
sec_to_day  = 1.1574074074074073e-05
sec_to_year = 3.168e-8
year_to_sec = 3.1559626e7
au_to_cm    = 1.49597871e13

# astronomical constants
parsec = 3.08e18    # parsec in cm
kpc    = 3.08e21
mpc    = 3.08e24
m_sun  = 1.99e33
r_sun  = 6.995e10
L_sun  = 3.828e33 #  erg/s 

# functions

def PlanckFunctionWavelength(lam,T_bb):
    try:
        return 2. * h * pow(c,2.)/( pow(lam,5) * (exp(h * c /(lam * k * T_bb)) - 1.) )
    except:
        if (h * c /(lam * k * T_bb) > 20.):
            return 0.
        else:
            print "Error, check Planck function"

def PlanckFunctionFrequency(nu,T_bb):
    try:
        return 2. * h * pow(nu,3.)/( pow(c,2) * (exp(h * nu /(k * T_bb)) - 1.) )
    except:
        if (h * nu /(k * T_bb) > 20.):
            return 0.
        else:
            print "Error, check Planck function"
