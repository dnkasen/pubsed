...
variables below. 
    ...
    Usage: python convert_database.py
    ...
    with h5py.File('cmfgen_data.hdf5', 'w') as h5f:
        for species, species_data in iteritems(data_sources):
            ...
            for ion, ion_data_file in iteritems(species_data):
                ...
                ion_group.attrs['n_levels'] = s.nlevels
                ion_group.attrs['n_lines']  = s.nlines
                ion_group.attrs['ion_chi']  = s.ion_chi
                
                ion_group.create_dataset("level_i", data=s.lev)
                ion_group.create_dataset("level_g", data=s.g)
                ion_group.create_dataset("level_E", data=s.E)

                ion_group.create_dataset("level_measured", data=s.measured)
                ion_group.create_dataset("level_orbitals", data=s.orbitals)
                ion_group.create_dataset("level_parity",   data=s.parity)
                
                ion_group.create_dataset("line_l", data=s.line_l)
                ion_group.create_dataset("line_u", data=s.line_u)
                ion_group.create_dataset("line_A", data=s.line_A)
