...
//   " " << my_zone_stop_ - my_zone_start_ << "\n";
  ...
  store_Jnu_labframe_ = params_->getScalar<int>("transport_store_Jnu_labframe");

    // sanity check
  if (use_nlte_&&(!store_Jnu_cmf_))
    std::cerr << "WARNING: not storing Jnu cmf while using NLTE; Bad idea!\n";

  // more checks
  if (store_Jnu_cmf_ && !store_Jnu_)
    {
  ...
  if (store_Jnu_cmf_ || store_Jnu_labframe_) n_freq_variables += 1;
