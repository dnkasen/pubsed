...
def compare(pdf):
    ...
    data = py.loadtxt('plt_00001.dat',skiprows=2)
    r    = data[:,0]
    r    = r - 0.5*(r[1] - r[0])
    tgas = data[:,3]
    trad = data[:,4]
    ...
    py.title('spherical lightbulb test: radiation field')
    py.xlabel('radius (cm)')
    ...
    py.title('spherical lightbulb test: output spectrum')
    ...
    nu = py.array(fin['nu'])
    Jnu = py.array(fin['zonedata/50/Jnu'])
    rz = py.array(fin['r'])
    fin.close()
    py.plot(nu,Jnu,'o')
    ...
    py.legend(['sedona','analytic blackbody'])
    py.title('spherical lightbulb test: Jnu at zone=50')
    py.xlabel('frequency (Hz)')
    py.ylabel('Flux')
    py.yscale('log')
    py.xlim(0,3e15)
