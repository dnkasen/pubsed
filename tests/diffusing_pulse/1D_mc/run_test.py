...
def run_test(pdf="",runcommand=""):
    ...
    for i in range(1,n-1):
        esum += tr[i]**4*(r[i]**3.0-r[i-1]**3.0)*4.0/3.0*3.14159
    ...
    for i in [6,14,20]: 
        ...
        # analytic solution
        D = 3e10/rho/3.0
        Enorm =  E0*(1.0/(D*time*4.0*3.141519)**0.5)
        Ean = np.exp(-1.0*(r - r0)**2/(4.0*D*time)) - np.exp(-1.0*(r + r0)**2/(4.0*D*time))
        Ean = Ean*r0/r

        # total internal energy in pulse
        Enorm = 0.0
        for i in range(1,n-1):
            Enorm += Ean[i]*(r[i]**3.0-r[i-1]**3.0)*4.0/3.0*3.14159
        Ean = Ean/Enorm*esum

        Tan = Ean**0.25

#        print Tan
        plt.plot(r,Tan,color=colors[cnt],lw=3)
        cnt += 1
    ...
    plt.title('Diffusing Spherical Pulse -- Monte Carlo transport')
