...
def run_test(pdf="",runcommand=""):
    ...
    if (runcommand != ""):
        ...
        # run second ~half
        # first, have runcommand use a different number of ranks
        runcommand_components = runcommand.split()
        if runcommand_components[2] == '1':
          runcommand_components[2] = '2'
        else:
          runcommand_components[2] = '1'
        runcommand = ' '.join(runcommand_components)
...
#    if (h5diff_output): failure = 3
    ...
    ## make plot
    plt.title('1D Lucy Supernova test - monte carlo, checkpoint, rank count change')
