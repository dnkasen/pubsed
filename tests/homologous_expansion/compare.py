...
def compare(pdf):
	...
	x,y = np.loadtxt('ray_00001',unpack=1,usecols=[0,4])
	py.plot(x/r0,y,color=cols[0])
	x,y = np.loadtxt('ray_00010',unpack=1,usecols=[0,4])
	py.plot(x/r0,y,color=cols[1])
	x,y = np.loadtxt('ray_00020',unpack=1,usecols=[0,4])
	py.plot(x/r0,y,color=cols[2])
	x,y = np.loadtxt('ray_00030',unpack=1,usecols=[0,4])
	...
	for i in range(0,4):
		xn = x*(texp + i*10.0)/texp
		yn = y*texp/(texp + i*10.0)
