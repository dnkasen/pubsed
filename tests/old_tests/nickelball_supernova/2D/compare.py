...
def compare(pdf):
    ...
    for i in range(len(mu)):
        ...
        bol = py.zeros(nt)
        for it in range(nt):
            bol[it] = py.trapz(Lnu[it,:,i],x=nu)
        py.plot(times/3600.0/24.0,bol)

    xc,rc,lc = py.loadtxt('sedonabox_nickelball_lc.dat',usecols=[0,1,2],skiprows=1,unpack=1)
    ...
    f = h5py.File('sedonabox_nickelball.h5', 'r')
    ...
    # plot up the result
    py.plot(lam, Llam,linewidth=3,color='red')
    ...
    lam  = 2.99e10/nu*1e8
    py.plot(lam,smooth.smooth(Lnu[indt,:,0]*nu**2/2.99e10/1e8,1),color='black')
